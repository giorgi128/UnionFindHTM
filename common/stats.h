 ...
 *          get("num_updates")                                                  -> returns array containing all values stored for num_updates (probably per-std::thread values)
 *          get("(SUM ALL num_updates)")                                        -> returns array with a[0] = the sum of num_updates over all threads&indices
 *          get("(SUM BY_THREAD num_updates)")                                  -> returns array containing per-std::thread sums of num_updates
 ...
 *          get("[(SUM BY_THREAD num_updates) / " + str(elapsed_millis/1000.) + "]")        -> returns array containing per-std::thread throughputs
 *          get("[(STDEV BY_THREAD num_updates) / " + str(elapsed_millis/1000.) + "]")      -> returns array containing per-std::thread standard deviations for throughputs
 *          get("[[(STDEV BY_THREAD num_updates) / " + str(elapsed_millis/1000.) + "] / [(SUM BY_THREAD num_updates) / " + str(elapsed_millis/1000.) + "]]")    -> returns array containing per-std::thread standard deviations for throughputs, expressed as a percentage of the std::thread's throughput
 ...
 *          get("(HIST_LOG BY_THREAD range_query_sizes)")                       -> returns array containing per-std::thread arrays, each of which represents a logarithmic histogram of the values in range_query_size over all indices
 */
...
// todo: later, expand this to work with tables of data, not just arrays indexed by std::thread and an arbitrary index (could use that arbitrary index to handle tables by concatenating table keys?)
...
namespace stats_ns {
    ...
    private:
        ...
        #define PRINT_LOWER(str) { std::string __s = str; transform(__s.begin(), __s.end(), __s.begin(), ::tolower); std::cout<<__s; }
        ...
        #define PRINT_AGG(agg_granularity_str, type, sid, metrics, num_metrics) { \
            ...
            if (metrics == NULL) { \
                ...
                for (int __i=0;__i<num_metrics;++__i) { \
                    std::cout<<(__i?" ":"=")<<metrics[__i].TYPE_TO_FIELD(type); \
                } \
                std::cout<<std::endl; \
            } \
        }
        #define PRINT_HISTOGRAM_LOG(sid, agg_granularity_str, type, metrics, num_metrics) { \
            ...
            PRINT_LOWER(#type); \
            std::cout<<" "<<id_to_name[sid]<<agg_granularity_str<<"="; \
            for (int __i=0;__i<=__last_nonzero;++__i) std::cout<<(__i?" ":"")<<(1<<__i)<<":"<<__histogram[__i].TYPE_TO_FIELD(type); \
            std::cout<<std::endl; \
            for (int __i=0;__i<=__last_nonzero;++__i) { \
                std::cout<<"    "<<(__i?"(":"[")<<"2^"<<twoDigits(__i)<<", 2^"<<twoDigits(__i+1)<<"]: "<<__histogram[__i].TYPE_TO_FIELD(type)<<std::endl; \
            } \
        }
        ...
        #define PRINT_HISTOGRAM_LIN(sid, agg_granularity_str, type, metrics, num_metrics, num_buckets) { \
            ...
            PRINT_LOWER(#type); \
            std::cout<<" "<<id_to_name[sid]<<agg_granularity_str<<"="; \
            for (int __i=0;__i<=(num_buckets);++__i) std::cout<<(__i?" ":"")<<(__dims.PASTE_MIN(TYPE_TO_FIELD(type)) + (1+__i)*__dims.PASTE_BUCKET_SIZE(TYPE_TO_FIELD(type)))<<":"<<__histogram[__i].TYPE_TO_FIELD(type); \
            std::cout<<std::endl; \
        ...
        //std::cout<<"    "<<(__i?"(":"[")<<(__dims.PASTE_MIN(TYPE_TO_FIELD(type)) + __i*__dims.PASTE_BUCKET_SIZE(TYPE_TO_FIELD(type)))<<", "<<(__dims.PASTE_MIN(TYPE_TO_FIELD(type)) + (1+__i)*__dims.PASTE_BUCKET_SIZE(TYPE_TO_FIELD(type)))<<"]: "<<__histogram[__i].TYPE_TO_FIELD(type)<<std::endl;
        
        const int NUM_PROCESSES;
        std::map<stat_id, std::string> id_to_name;
        std::map<std::string, stat_id> name_to_id;
        stats_thread_data * thread_data;
        stat_id num_stats;
        
        std::multimap<stat_id, stat_output_item> output_config;
...
//                    
//                    std::cout<<"std::thread "<<tid<<" copying slice starting at "<<myslice_start<<" with size "<<myslice_sz<<std::endl;
//                    
...
//            }
            ...
            for (int tid=0;tid<num_processes;++tid) {
                threads[tid] = std::thread([tid,&start,num_processes,this]() {
                    while (!start) { __sync_synchronize(); }
                    //std::cout<<"std::thread "<<tid<<" initializing thread_data entry of size "<<sizeof(stats_thread_data)<<std::endl;
...
//                computed_histograms_lin[id] = NULL;
        ...
        stat_id create_stat(enum_data_type datatype, std::string name, const int capacity, std::initializer_list<stat_output_item> stat_output_items) {
            ...
            for (auto output_item : stat_output_items) {
                output_config.insert(std::pair<stat_id, stat_output_item>(id, output_item));
            }
        ...
        inline T add_stat(const int tid, const stat_id id, T value, const int index) {
            ...
            //std::cout<<"adding to id="<<id<<" index="<<index<<" value="<<value<<" result="<<ptr[index]<<std::endl;
...
//            std::cout<<"appending to id="<<id<<" index="<<index<<" value="<<value<<" at index="<<index<<" result="<<ptr[index]<<std::endl;
    ...
    private:
        ...
        int log2_capped(T x) {
            ...
            //if (log2_x_i < 0 || log2_x_i > DEFAULT_HISTOGRAM_LOG_NUM_BUCKETS) std::cout<<"x="<<x<<" log2_x_i="<<log2_x_i<<std::endl;
#ifndef NDEBUG
        ...
        stat_metrics<long long> * get_histogram_log(const stat_id id, stat_metrics<T> * const metrics, const int num_metrics) {
            ...
            for (int thread_id=0;thread_id<std::thread::hardware_concurrency();++thread_id) {
                //std::cout<<"    parallel compute histogram: spawning std::thread "<<thread_id<<std::endl;
                threads[thread_id] = std::thread([thread_id, histogram, id, &start, num_buckets, metrics, this]() {
                    ...
                    // compute a std::thread-local histogram for only our slice of the indices
                    if (metrics) {
                        /**
                    ...
                    //std::cout<<"    parallel compute histogram: std::thread "<<thread_id<<" terminated"<<std::endl;
                });
                //std::cout<<"    parallel compute histogram: spawned std::thread "<<thread_id<<std::endl;
            }
            __sync_synchronize();
            start = true;
            __sync_synchronize();
            ...
            for (int i=0;i<std::thread::hardware_concurrency();++i) {
                //std::cout<<"    parallel compute histogram: joining std::thread "<<i<<std::endl;
                threads[i].join();
                //std::cout<<"    parallel compute histogram: joined std::thread "<<i<<std::endl;
            }
        ...
        template <typename T>
        std::pair<stat_metrics<long long> *, histogram_lin_dims> get_histogram_lin(const stat_id id, const int num_buckets, stat_metrics<T> * const metrics, const int num_metrics) {
            // compute histogram
...
//            std::cout<<"start compute_before_printing()..."<<std::endl;
            ...
            for (int id=0;id<num_stats;++id) {
                if (this->data_types[id] == LONG_LONG) {
                    threads[3*id+0] = std::thread([this,id]() { this->computed_stats_total[id] = (stat_metrics<double> *) compute_stat_metrics_total<long long>(id); });
                    // WARNING: compute_stat_metrics_INDEX SETS this->num_indices[id], WHICH IS NEEDED FOR GET_HISTOGRAM_... CALLS AND print_all!
                    threads[3*id+1] = std::thread([this,id]() { this->computed_stats_by_index[id] = (stat_metrics<double> *) compute_stat_metrics_by_index<long long>(id, &this->num_indices[id]); });
                    threads[3*id+2] = std::thread([this,id]() { this->computed_stats_by_thread[id] = (stat_metrics<double> *) compute_stat_metrics_by_thread<long long>(id); });
                } else {
                    threads[3*id+0] = std::thread([this,id]() { this->computed_stats_total[id] = compute_stat_metrics_total<double>(id); });
                    // WARNING: compute_stat_metrics_INDEX SETS this->num_indices[id], WHICH IS NEEDED FOR GET_HISTOGRAM_... CALLS AND print_all!
                    threads[3*id+1] = std::thread([this,id]() { this->computed_stats_by_index[id] = compute_stat_metrics_by_index<double>(id, &this->num_indices[id]); });
                    threads[3*id+2] = std::thread([this,id]() { this->computed_stats_by_thread[id] = compute_stat_metrics_by_thread<double>(id); });
                }
            }
